...
export function SettingsApi() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 p-8">
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-5">
        <h2 className="mb-1 text-[14px] font-semibold text-gray-900">Self-curation</h2>
        <p className="mb-4 text-[12.5px] leading-relaxed text-gray-500">
          CRYS infers bridging assumptions between crystals — held out of
          recall until you approve them. Explore mode also tests pairs
          found by structure alone (shared sources, similarity); off means
          only pairs your actual usage connected are tested.
        </p>
        <div className="flex items-center gap-2">
          <span className="min-w-0 flex-1 text-[13px] text-gray-700">
            Explore assumptions beyond demand evidence
            <span className={"ml-2 rounded-full px-2 py-0.5 text-[11px] font-medium " +
              ((record.data?.assumptions_explore ?? true)
                ? "bg-emerald-50 text-emerald-700"
                : "bg-gray-100 text-gray-500")}>
              {(record.data?.assumptions_explore ?? true) ? "on" : "off"}
              {record.data?.assumptions_explore == null ? " — default" : ""}
            </span>
          </span>
          <div className="flex shrink-0 items-center gap-2">
            <button
              disabled={busy !== null}
              onClick={() =>
                void act("explore", async () => {
                  const next = !(record.data?.assumptions_explore ?? true);
                  await api.setAssumptionsExplore(selectedCustomerId!, next);
                  await record.refetch();
                }, "Self-curation setting saved.")}
              className="rounded-lg bg-gray-900 px-3.5 py-2 text-[12.5px] font-semibold text-white transition hover:bg-gray-700 disabled:opacity-40"
            >
              {busy === "explore"
                ? "Saving…"
            ...
            {record.data?.assumptions_explore != null && (
              <button
                disabled={busy !== null}
                onClick={() =>
                  void act("explore_reset", async () => {
                    await api.setAssumptionsExplore(selectedCustomerId!, null);
                    await record.refetch();
